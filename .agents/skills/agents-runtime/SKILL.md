...
3. Every agent that calls an LLM goes through `agents.lib.llm.complete(...)` — never imports `groq` or `cerebras` directly.
...
from agents.lib.llm import complete
...
class <Name>Agent(BaseAgent[<Name>Input, <Name>Output]):
    ...
    async def run(self, input: <Name>Input) -> <Name>Output:
        ...
        return <Name>Output.model_validate_json(reply)
```
...
| LLM fallback never triggers | Check that your exception classes inherit from `LLMRateLimitError` — `complete()` catches by type |
